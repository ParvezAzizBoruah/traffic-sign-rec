...
for i in range(6):
    ...
    sc=cv2.compareHist(histt, hist2, cv2.HISTCMP_CHISQR)  #compare the histogram using Chisqr method
    print(sc)
    del im
    del hist2
i=0
...
for i in range(4):
    ...
    hist2 = cv2.calcHist([im], [0], None, [256], [0, 256])  #calculate the histogram of all the images one-by-one
    sc=cv2.compareHist(histt, hist2, cv2.HISTCMP_CHISQR)  #compare the histogram using Chisquare method
    print(sc)
    del im
    del hist2
